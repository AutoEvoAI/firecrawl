...
export async function scrapeURLWithFetchStealth(
  meta: Meta,
): Promise<EngineScrapeResult> {
  const mockOptions = {
    url: meta.rewrittenUrl ?? meta.url,
  ...
  if (meta.mock !== null) {
    ...
    try {
      const x = await undici.fetch(meta.rewrittenUrl ?? meta.url, {
        dispatcher: getStealthDispatcher(meta.options.skipTlsVerification),
        redirect: "follow",
        headers: meta.options.headers,
        signal: meta.abort.asSignal(),
      });
  ...
  await specialtyScrapeCheck(
    meta.logger.child({ method: "scrapeURLWithFetchStealth/specialtyScrapeCheck" }),
    Object.fromEntries(response.headers as any),
  );
