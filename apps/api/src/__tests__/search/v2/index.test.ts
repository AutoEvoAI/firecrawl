...
describe("search/v2/index", () => {
  ...
  describe("parallel search", () => {
    it("should execute parallel search when AI mode is enabled and multiple queries are provided", async () => {
      mockedSearxngSearch
        .mockResolvedValueOnce({
          web: [{ url: "http://example1.com", title: "Result 1", description: "Description 1" }],
        })
        .mockResolvedValueOnce({
          web: [{ url: "http://example2.com", title: "Result 2", description: "Description 2" }],
        })
        .mockResolvedValueOnce({
          web: [{ url: "http://example3.com", title: "Result 3", description: "Description 3" }],
        });
      ...
      expect(mockedSearxngSearch).toHaveBeenCalledTimes(3);
      expect(result.web).toBeDefined();
      expect(result.web?.length).toBeGreaterThan(0);
    });

    it("should limit parallel queries to MAX_PARALLEL (4)", async () => {
      ...
      // Should only call 4 times (MAX_PARALLEL)
      expect(mockedSearxngSearch).toHaveBeenCalledTimes(4);
    });

    it("should pass AI metadata to SearXNG adapter", async () => {
      mockedSearxngSearch.mockResolvedValue({ web: [] });
    ...
    it("should handle errors in parallel search gracefully", async () => {
      mockedSearxngSearch
        .mockResolvedValueOnce({
          web: [{ url: "http://example.com", title: "Result", description: "Description" }],
        })
    ...
    it("should not use parallel search for single query", async () => {
      mockedSearxngSearch.mockResolvedValue({ web: [] });
      ...
      // Should use regular search path (single call)
      expect(mockedSearxngSearch).toHaveBeenCalledTimes(1);
    });

    it("should not use parallel search when AI mode is false", async () => {
      mockedSearxngSearch.mockResolvedValue({ web: [] });
      ...
      // Should use regular search path
      expect(mockedSearxngSearch).toHaveBeenCalledTimes(1);
    });
  });
